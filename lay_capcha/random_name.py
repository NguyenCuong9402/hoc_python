...
# Hàm để đọc proxy từ tệp và trả về danh sách proxy
...
def create_accounts(num_accounts, proxys):
    accounts = []
    index = 1
    # for index in range(num_accounts):
    while len(accounts) < num_accounts:
        print(f"{len(accounts)}-----{index}-------------------------------------------------------------------------------------------------------")
        ...
        if response.status_code == 200:
            ...
            if image_base64:
                ...
                response_register = requests.post(api_login, json=payload, proxies={'http': proxy})
                if response_register.status_code == 200:
                    ...
                    if data.get('Result') is not None and data.get('Result') != {}:
                        ...
                        print("Đăng ký - không thành công", data)
                        index += 1
            ...
            print("Failed to retrieve captcha. Status code:")
...
create_accounts(num_accounts, proxies)